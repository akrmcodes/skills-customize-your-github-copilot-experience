...
Build a command-line Hangman game and strengthen your Python fundamentals, including loops, conditionals, strings, lists, and user input.
...
#### Description
Set up the initial game in `starter-code.py` by selecting a secret word, creating variables to track guesses, and showing the player the starting progress.
...
#### Description
Implement the main game loop where the player enters guesses, the game state updates each turn, and the game ends with a clear result.
...
- Handle repeated guesses clearly so the player knows to try a different letter.
